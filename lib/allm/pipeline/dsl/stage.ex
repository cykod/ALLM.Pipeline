defmodule ALLM.Pipeline.Dsl.Stage do
  ...
  | `carry` | field names captured into the context's carry map — `stage` only, see below |
  ...
  | `on_error` | `kind: :stage`: `:fail_run` (default) or `:continue`. It governs a whole-stage failure; a `fan_out` has none (its items fail individually into its output), so declaring it there is a compile error |
